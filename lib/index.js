...
function containerFlow(parent, state, info) {
  ...
  while (++index < children.length) {
    ...
    if (child.type === 'mdxJsxFlowElement') {
      serializedChild = result
    } else if (String(child.type) === 'mdxFlowExpression') {
      // Expressions keep inner whitespace (JS strings). Only indent `{` and a
      // closing `}` so JSX indent does not accumulate on each roundtrip.
      serializedChild = state.indentLines(
        result,
        function (line, lineIndex, blank) {
          return !blank && (lineIndex === 0 || line === '}')
            ? currentIndent + line
            : line
        }
      )
    } else {
      serializedChild = state.indentLines(result, function (line, _, blank) {
